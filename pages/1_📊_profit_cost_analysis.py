...
with col[0]:
    ...
    def compute_metrics(df):
        ...
        annual_profit = (filtered_df.groupby(filtered_df['Month'].dt.year)['Profit']
                         .sum()
                         .reset_index(name='Profit'))
        annual_profit['profit_change'] = annual_profit['Profit'].pct_change() * 100
        
        annual_cost = (filtered_df.groupby(filtered_df['Month'].dt.year)['Cost']
                       .sum()
...
with col[1]:
    st.markdown("<h5 style='text-align: center;'>Cost, Sales and Profit Over Time</h5> &nbsp;", unsafe_allow_html=True)
    ...
    chart = alt.Chart(df_melted).mark_bar().encode(
        ...
        color=alt.Color('Category:N', scale= alt.Scale(scheme=selected_color_theme), legend=alt.Legend(title="Category")),
        tooltip=[alt.Tooltip('Month:T', title='Month'), alt.Tooltip('Category:N', title='Category'), alt.Tooltip('Value:Q', title='Amount')]
    ).properties(
        width=800,
        height=400,
    ).configure_axis(
...
with col2:
    ...
    line_chart = alt.Chart(df_melted).mark_line(point=True).encode(
        ...
        color=alt.Color('Category:N', scale= alt.Scale(scheme=selected_color_theme), legend=alt.Legend(title='Metric')),
...
with col3:
    st.markdown("<h5 style='text-align: center;'>Expenditure Distribution</h5>", unsafe_allow_html=True)
    ...
    color=alt.Color(field="Expenditure", type="nominal", legend=alt.Legend(title="Expenditure Type"),
